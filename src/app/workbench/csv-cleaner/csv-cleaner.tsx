...
export function CsvCleaner() {
  ...
  return <div className="space-y-6">
    <section className="rounded-2xl border border-neutral-800 bg-neutral-900/40 p-5 sm:p-7" aria-label="CSV input">
      ...
      <label className="mt-5 block text-sm text-neutral-300">Or paste CSV<textarea disabled={loading} maxLength={MAX_CSV_BYTES} spellCheck={false} value={source} onChange={event => edit(event.target.value)} className="mt-2 block min-h-48 w-full rounded-xl border border-neutral-700 bg-neutral-950 p-4 font-mono text-sm text-neutral-100" placeholder="name,email,team" /></label>
