...
export function AccessEditor({ subject, app, grant }: { subject: string; app: PlatformApp; grant?: AppAccess }) {
  ...
  return <form aria-label={`Access to ${app.name}`} className="rounded-xl border border-neutral-700 p-5" onSubmit={async event => {
    ...
    <fieldset disabled={busy} className="mt-5 grid gap-4 sm:grid-cols-2"><legend className="sr-only">Application access settings</legend>
      <label className="text-sm">Access<select name="status" defaultValue={grant?.status ?? "revoked"} className="mt-2 block w-full rounded-lg border border-neutral-700 bg-neutral-950 p-3"><option value="enabled">Enabled</option><option value="suspended">Suspended</option><option value="revoked">Revoked</option></select></label>
      <label className="text-sm">Application role<select name="app_role" defaultValue={grant?.app_role ?? "user"} className="mt-2 block w-full rounded-lg border border-neutral-700 bg-neutral-950 p-3"><option value="user">User</option><option value="app_admin">Application administrator</option></select></label>
